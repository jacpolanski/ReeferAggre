    ...
    data.containers.forEach((container) => {
      container.Supply.forEach((reading) => {
        reading !== "" &&
          ...
          reading > container.TempSP + threshold.supply &&
          (alarms[0] += 1);
        reading === "" && (alarms[2] += 1);
      });
      container.Return.forEach((reading) => {
        reading !== "" &&
          threshold.supply !== 0 &&
          typeof container.TempSP === "number" &&
          reading > container.TempSP + threshold.return &&
          (alarms[1] += 1);
        reading === "" && (alarms[2] += 1);
      });
    });
    ...
    return (
      <Card>
        <Card.Body className="d-flex flex-column justify-content-center">
          <Card.Title className="fs-2 mb-5">Alarms today</Card.Title>
          <Card.Subtitle className="mb-5 text-muted fs-3">
            Total alarms:{" "}
            <Spinner animation="grow" variant="primary" size="sm" /> units
          </Card.Subtitle>
