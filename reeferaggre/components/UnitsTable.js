...
const UnitsTable = ({datesToShootString}) => {
    ...
    useEffect(() => {
        if (show === true) {
                ...
                plugins: {
                    ...
                    annotation: {
                        annotations: {
                            ...
                            setPoint: {
                                type: 'line',
                                yMin: tableData.containers.filter(container => container.ContainerID === modalContID)[0].TempSP,
                                yMax: tableData.containers.filter(container => container.ContainerID === modalContID)[0].TempSP,
                            ...
                            supply: {
                                ...
                                yMin: tableData.containers.filter(container => container.ContainerID === modalContID)[0].TempSP + threshold.supply,
                                yMax: tableData.containers.filter(container => container.ContainerID === modalContID)[0].TempSP + threshold.supply,
                                borderDash: [6, 6],
                                borderDashOffset: 0,
                                borderWidth: 3,
                                label: {
                            ...
                            return: {
                                ...
                                yMin: tableData.containers.filter(container => container.ContainerID === modalContID)[0].TempSP + threshold.return,
                                yMax: tableData.containers.filter(container => container.ContainerID === modalContID)[0].TempSP + threshold.return,
                                borderDash: [6, 6],
                                borderDashOffset: 0,
                                borderWidth: 3,
                                label: {
            ...
            setData({
                labels: tableData.dates.reduce((arr, date) => (date !== "") ? [...arr, `${date} AM`, `${date} PM`] : [...arr], []),
                datasets: [
                    {
                        label: 'Supply',
                        data: tableData.containers.filter(container => container.ContainerID === modalContID)[0].Supply.map(temp => (temp !== "") ? temp : null),
                    ...
                    {
                        label: 'Return',
                        data: tableData.containers.filter(container => container.ContainerID === modalContID)[0].Return.map(temp => (temp !== "") ? temp : null),
        ...
        <>
            ...
            <Modal show={show} fullscreen={true} onHide={() => setShow(false)}>
                ...
                <Modal.Body className="p-1">
                    <Table striped bordered hover size="sm" className="table-fixed text-center mx-auto w-50">
                        <thead className="sticky-header">
                        <tr className="sticky-header">
                            <th className="sticky-header" colSpan={2}>Standard Info</th>
                            <th className="sticky-header" colSpan={2}>Manifest</th>
                            <th className="sticky-header" rowSpan={2}>Temp SP</th>
                        </tr>
                        <tr className="sticky-header">
                            <th className="sticky-header">Location</th>
                            <th className="sticky-header">ContainerID</th>
                        ...
                        <tbody>
                        {tableData.containers.filter(container => container.ContainerID === modalContID).map(container => (
                            <tr key={uniqid()}>
                                <td key={uniqid()}>{container.Locations}</td>
                    ...
                    <div className="w-75 mx-auto">
                        {(data === undefined) ? <p> Loading ...</p> : <Line options={options} data={data}/>}
                    </div>
                </Modal.Body>
            </Modal>
        </>
